...
trait GenerateSchema
{
     ...
     * @return mixed
     */
    public function createSchema ($db, $createXmlFile = false)
    {
        $this->conn_name = $db;
    ...
    private function generateForeign ($table)
    {
        $stmt2 =  $this->conn->query("SELECT i.TABLE_NAME, i.CONSTRAINT_TYPE, i.CONSTRAINT_NAME, k.REFERENCED_TABLE_NAME,
                                      ...
                                      WHERE i.CONSTRAINT_TYPE = 'FOREIGN KEY' AND i.TABLE_NAME = '$table' AND i.TABLE_SCHEMA = DATABASE() AND k.CONSTRAINT_SCHEMA = DATABASE();");
