...
//                    }
    ...
    /**
     * Faz o dataBinding da model.
     * @author Hiago Silva Souza <hiasilva@gmail.com>
     * @param $data
     * @param \ReflectionClass $c
     * @return object
     */
    public static function modelBind($data,\ReflectionClass $c) {

        $class = $c->newInstanceWithoutConstructor();

        foreach ($c->getProperties() as $p) {
            $nomeProp = $p->getName();
            if (isset($data->{$nomeProp}) && method_exists($class, 'set'.Utils::snakeToCamelCase($p->getName()))) {
                $class-> {'set'.Utils::snakeToCamelCase($p->getName())} ($data->{$nomeProp});
            }

            //verificando se é uma model
            $info = AnnotationReader::getAnnotationProperty($class, $p->getName());
            if(isset($info['one_to_one']) && $info['one_to_one'] !== 'false') {
                $method = 'set' . Utils::snakeToCamelCase($info['one_to_one']) . Utils::snakeToCamelCase($p->getName()) . 'Model';
                if(method_exists($class, $method) && isset($data->{$info['one_to_one'].'Object'})) {

                    $ndata = $data->{$info['one_to_one'].'Object'};
                    $cstr = "Model\\" . Utils::snakeToCamelCase($info['one_to_one']) . "Model";
                    $r = new \ReflectionClass($cstr);

                    $nc = Utils::modelBind($ndata, $r);
                    $nc->save();
                    $class->$method($nc);
                }
            }
