...
trait GenerateClass
{
            ...
            if ($table->hasChildNodes()) {
                ...
                foreach ($table->childNodes as $columns) {
                    ...
                    if ($columns->tagName == 'column') {
                        ...
                        $columnType = $columns->getAttribute('type');
                        $size = $columns->getAttribute('size');
                        if (empty($size))
                            $size = 'null';

                        $required = $columns->getAttribute('required');
                        $primaryKey = $columns->getAttribute('primaryKey');
                        if (empty($primaryKey))
                            $primaryKey = 'false';

                        $autoIncrement = $columns->getAttribute('autoIncrement');
                        ...
                        $oneToOne = "false";
                        foreach ($this->foreignTable as $key => $chave) {
                            if ($chave == $columnName && $primaryKey == 'true') {
                                $oneToOne = $key;
                            }
                        }
                        ...
                        $this->methodSet($columnName, $columnType);
                        $this->methodGet($columnName, $columnType);
                        ...
                        foreach ($columns->childNodes as $fk) {
                            ...
                            if (empty($this->foreignTable[$columns->getAttribute('foreignTable')])) {
                                ...
                                $uses .= "use " . str_replace("/", "\\", $this->dir) . "\\" . Utils::snakeToCamelCase($columns->getAttribute('foreignTable')) . "Model;\n";
                            }
     ...
     */
    private function methodSet($columnName, $columnType = null)
    {
        ...
        //verificando se é uma coluna do tipo data
        if ($columnType == 'datetime') {
            $v .= "\n\t\tif(strpos('.',\$val) !== false) { ";
            $v .= "\n\t\t\t\$this->" . lcfirst($name) . ' = date("Y-m-d H:i:s", strtotime(explode(".",$val)[0]));';
            $v .= "\n\t\t} else { ";
            $v .= "\n\t\t\t\$this->" . lcfirst($name) . ' = !empty($val) ? date("Y-m-d H:i:s", strtotime($val)) : $val;';
            $v .= "\n\t\t}";
        } else {
     ...
     * @param string $columnName ;
     */
    private function methodGet($columnName, $columnType = null)
    {
        $name = Utils::snakeToCamelCase($columnName);
        ...
        $v .= "\n\t{";
        if ($columnType == 'datetime') {
            $v .= "\n\t\treturn !empty(\$this->" . lcfirst($name) . ") ? date('Y-m-d\\TH:i:s', strtotime(\$this->" . lcfirst($name) . ")) : \$this->" . lcfirst($name) . ";";
        } else {
    ...
    private function structureTrait($use = null)
    {
        ...
        $v .= "\n{";
        $v .= "\n\tpublic static \$conn_name = \"".$this->conn_name."\";";
        $v .= "\n$this->propertys";
        $v .= "\n$this->methodsSeters";
        $v .= "\n$this->methodsGeters";
        $v .= "\n}";
    ...
    private function getAftersAndBefores()
    {
        $v = "\n\n\t//region Methods Afters and Befores";
        $v .= "\n\tprotected function beforeSave() {";
        $v .= "\n\t\t//TODO Method auto generated";
        $v .= "\n\t}";
